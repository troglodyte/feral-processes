...
Perks are the player's own permanent upgrades, and the only progression that is spent rather than
earned twice. Every level hands you two Perk Points; the Perks row on the party menu is where they
go, from anywhere, at any time.

A perk is bought over and over. Each purchase stacks another level on the last and costs exactly
...
The first points of a run are best spent on whatever your run is short of rather than on a plan.
Power drain and roster room are the two shortages a new player meets earliest; the affinity
multipliers only start paying once you have routines worth multiplying.

Read on: [getting stronger](getting-stronger), [routines](routines), and [the Stack](the-stack).

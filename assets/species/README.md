...
(
    ...
    // editing — but the engine converts each one into an ability at load
    // (`species::basic_attack_ability`), so combat sees a single kind of
    // thing. A converted attack is priced at nothing, has no cooldown, and is
    // kept out of the pool a Routine Disk rolls from, so basic attacks stay
    // enemy-only. The damage a basic attack deals is deliberately *not*
    // computed the way a Special's is — see
    // `seam:a-basic-attack-is-an-ability-the-two-are-applied-by`.
    moves: [

    ...
    pub(crate) fn handle_downed_programs_key(&mut self, key: GameKey) {
        ...
        match self.pending_downed_program_index {
            ...
            Some(program_index) => {
                ...
                // **This screen is the player's own hands and nothing else.**
                // A rig is fed by a posted body fetching from a Quarantine
                // Rack, and it strips with the tool fitted to it — so there
                // is no second verb here handing the pack to a machine, and
                // no way for the two to disagree about which tool a

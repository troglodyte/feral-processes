...
impl Game {
    ...
    /// its level.
    pub(crate) fn downed_program_label(&self, program: &DownedProgram) -> String {
        let name = self
            .world
            .resource::<SpeciesDb>()
            .get(&program.species)
    ...
    /// is a physical handover, and `take_from_adjacent` / `give_to_adjacent`
    /// are the rule for those. Sorted by tile before the pick,

...
//!
//! Content, the way `descriptions.rs` and `sectors.rs` are: a `.ron` line
//! bank under `assets/nemesis/` — `names.ron` — loaded by
//! `NemesisDb::load_dir` with the same skip-and-warn discipline every other
//! `load_dir` in this crate follows.
//!
...
//! whose output is not guaranteed stable across a `rand` upgrade. So the
//! pick folds the values it is derived from and reduces with
...
/// same shared-voice call the design doc makes (only 4 of 17 shipped
/// species author `SpeciesDef::taunts` today, so a per-species nemesis pool
/// would read generic for most of the roster).
...
/// The name pool, loaded from `assets/nemesis/`.
///
/// **An empty database is valid and inert**, exactly like `SectorDb`:
/// `name` returns `None`, and `Game::mark_nemeses` already treats that as
/// "mark it, promote it, recharge it — just don't rename it" rather than a
/// fault. Deleting `assets/nemesis/` restores the pre-nemesis game's
/// naming, the same supported way to play that deleting `assets/sectors/`
/// or `assets/policies/enemy_battle.ron` is.
...
impl NemesisDb {
    /// Loads `names.ron` from `dir`. Follows every other `load_dir` in the
    /// crate: a missing or malformed file is skipped with a warning rather
    /// than aborting startup — see `DescriptionDb::load_dir`.
    pub fn load_dir(dir: &Path) -> std::io::Result<(Self, Vec<String>)> {
        let mut warnings = Vec::new();
        let names = load_bank(&dir.join("names.ron"), &mut warnings);
        Ok((NemesisDb { names }, warnings))
    }
...
/// Indexes `pool` by `seed`, or `None` when the pool is empty — legal, and
/// how a mod-free/partially-authored install behaves.

...
pub(in crate::render) fn regions(
    ...
    // whichever is larger. **One clearance is enough because the map pane's
    // bottom border carries nothing** — the compass, which briefly rode it,
    // is a block inside the pane now (`hud::compass_block`); a second strip
    // reaching down into this gap is what would need two.
...
mod tests {
    ...
    ///
    /// `map_pane`'s bottom border carries nothing at all, which is why this
    /// asserts about the log pane's top instead of flipping the sign on the
    /// old one.
    ///

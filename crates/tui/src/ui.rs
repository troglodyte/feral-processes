...
fn render_help(f: &mut Frame) {
    ...
    let lines = vec![
        ...
        Line::from(""),
        Line::from(
            "In an intrusion:  a attack   d defend   s special   c decompile (needs a taming catalyst)",
        ),
        Line::from("                  u use item   j jack out"),
        Line::from("                  A all attack   D all defend (shift = the whole party)"),
        Line::from(""),
        ...
        Line::from(""),
        Line::from("Up to 3 companions (p) can fight alongside you at once. A party"),
        Line::from("member's Special (s) doesn't attack — it buffs you instead of that"),
        Line::from("member acting that round: a temporary Attack rally by default, or its"),
        Line::from("species' own special ability if it has one (a bigger buff, a heal, or a"),
        Line::from("debuff on the wild program). Every member picks its own action. The"),
        ...
        Line::from("are left behind, and there's no portal back down. A Zone Portal costs 10"),
        Line::from("Portal Fragments times your current zone level to deploy."),
        Line::from(""),

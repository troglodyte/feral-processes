...
- [ ] **Step 1: Write the failing tests.** In `crates/engine/src/tests/`:
...
- [ ] **Step 3: Add the constants** to `tuning.rs`'s quality section, beside
...
- [ ] **Step 4: Add `Game::best_structure_tier`** to `game/catalog.rs`,
...
- [ ] **Step 5: Run the tests, mutation-check them** (delete the `max`, watch
...
- [ ] **Step 1: Write the failing tests.** The floor rises by
      ...
      rule).
- [ ] **Step 2: Run them and watch them fail.**
- [ ] **Step 3: Carry the bench on `CraftRecipe`.** A
...
- [ ] **Step 4: Add `CraftOrder` and its two writers** in
...
- [ ] **Step 1: Write the failing tests.** Careful charges each line's
...
- [ ] **Step 2: Run them and watch them fail.**
- [ ] **Step 3: Thread `careful` through the three signatures.**
      ...
      real flag in Task 5.
- [ ] **Step 4: Run the engine suite, mutation-check, commit.**

## Task 4: `craft` rolls a copy per unit

**Intent:** the behaviour change.

- [ ] **Step 1: Write the failing tests.** Compiling one piece of equipment
...
- [ ] **Step 2: Run them and watch them fail.**
- [ ] **Step 3: Roll in `craft`.** Build the `CraftOrder` once, then per unit
...
- [ ] **Step 4: Run the tests, mutation-check, then the full engine suite
...
- [ ] **Step 1: Write the failing app-core tests.** `[C]` on the
...
- [ ] **Step 2: Run them and watch them fail.**
- [ ] **Step 3: Add the flag and the key** in `app-core`: the field beside
      `pending_craft`, cleared in `handle_craft_key` where the page opens,
      toggled in `handle_craft_quantity_key`, threaded through
      `commit_craft`.
- [ ] **Step 4: Draw it** in `render/crafting.rs::draw_craft_quantity`: the
...
- [ ] **Step 5: Run the app-core and gui suites**, including the popup width
      tests — the new rows are `Row::Text`, so they do not touch the item
      column, but the footer line grows.
- [ ] **Step 6: Mutation-check the toggle** (delete the clear-on-open, watch
...
- [ ] **Step 1: Widen the census first and watch it fail.**
...
- [ ] **Step 2: Add `upgrade:` to `fabricator.ron` and `armory.ron`**,
...
- [ ] **Step 3: Run the asset censuses and the structures suite.**
- [ ] **Step 4: Commit.**

## Task 7: The gates, the seam and the rule

- [ ] **Step 1: Run the full gate** — `cargo fmt`, `cargo clippy
...
- [ ] **Step 2: Write the argument into `docs/seams.md`** — the crafter
      ...
      finding.
- [ ] **Step 3: Put the rule in `CLAUDE.md`** under **Items, gear and
      economy**, and `cp CLAUDE.md AGENTS.md` in the same commit.
- [ ] **Step 4: Tick the roadmap's phase table.**
- [ ] **Step 5: Commit.**

...
- [x] **3.1** `render/level_up.rs`, through `Painter` only, with fixed rows and no scroll. The layout is the spec's mock-up: three section headers and one row per figure. Format percentages as whole numbers and per-swing as one decimal. Mirror `notify.rs`'s panel and `draw_playing_base` underlay.
...
- [x] **3.3** Add the correction 10 gui sites and bump `ALL_MODES` to 119. Run `rg -n 'Mode::Notification' crates/gui/src` and check that every hit has a `LevelUp` twin.
- [x] **3.4** Docs. Draft a CHANGELOG entry under the existing convention, without a section header. If the `assets/help/` page on levelling names the level-up log lines, add a sentence about the page there. `docs/manual.md` and the root README are carved out, so leave them alone. Add a seam if one emerged. The likely candidate is "the level-up page's before column is a stored `Combatant`, never a re-derivation at the old level", written three places per the `seams` skill.
- [x] **3.5** Run `cargo test --workspace`. Commit. Tell the user it is unplaytested; agents have no display.
